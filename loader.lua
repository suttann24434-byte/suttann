...
local function OpenMainGUI()
    ...
    AddToggle("FPS Boost", function(state)
        if state then
            ...
            StarterGui:SetCore("SendNotification",{Title="FPS Boost",Text="Nonaktif tidak tersedia (Infinity Yield aktif)",Duration=4})
        end
    end)
        ...
        if state then
            for _,plr in pairs(Players:GetPlayers()) do
                if modAdminOwnerList[plr.Name] then
                    StarterGui:SetCore("SendNotification",{Title="Mod/Admin Detected",Text=plr.Name.." ada di server!",Duration=5})
                end
            end
            Players.PlayerAdded:Connect(function(plr)
                if modAdminOwnerList[plr.Name] then
                    StarterGui:SetCore("SendNotification",{Title="Mod/Admin Detected",Text=plr.Name.." join server!",Duration=5})
                end
            end)
        end
    end)
